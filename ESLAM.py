...
class ESLAM():
    ...
    def __init__(self, cfg):
        ...
        self.truncation = cfg['model']['truncation']#0.06
        ...
        self.estimate_c2w_list = torch.zeros((self.n_img, 4, 4), device=self.device)#Out of memory
        ...
        for shared_planes in [self.shared_planes_xy, self.shared_planes_xz, self.shared_planes_yz]:
            for i, plane in enumerate(shared_planes):
                plane = plane.to(self.device)#out of memory
                plane.share_memory_()
                shared_planes[i] = plane
