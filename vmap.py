...
def origin_dirs_W(T_WC, dirs_C):#T_WC: camera pose
    ...
    #np.savetxt("dirs_C.txt", dirs_C.clone().reshape(-1,3).cpu().numpy())
...
class sceneObject:
    ...
    def __init__(self, cfg, obj_id, rgb:torch.tensor, depth:torch.tensor, mask:torch.tensor, bbox_2d:torch.tensor, t_wc:torch.tensor, live_frame_id) -> None:
        ...
        if self.do_bg and self.obj_id == 0:
            self.keyframe_buffer_size = cfg.keyframe_buffer_size*20
        else: 
    ...
    def get_bound_from_frames(self,cfg, scale=1):
        ...
        end_time = time.time()
        print("time to get volume:", end_time - start_time)
        ...
        for kf_id in range(self.n_keyframes):
            ...
            end_time = time.time()
            print("13:", end_time - start_time)
            
            start_time = time.time()
            volume.integrate(rgbd, intrinsic, w2c)
            end_time = time.time()
            print("14:", end_time - start_time)


        end_time_loop = time.time()
        print("time of for loop:", end_time_loop - start_time_loop)
        ...
        end_time = time.time()
        print("time to get o3d_pc:", end_time - start_time)
        ...
        end_time = time.time()
        print("time to get mesh:", end_time - start_time)
        ...
        end_time = time.time()
        print("!!!Time to get mesh bound: ",end_time-start_time, "s", "keyframe length:", self.n_keyframes)
        return_mesh.export('./mesh_bound.obj')
    ...
    def get_training_samples(self, n_frames, n_samples, cached_rays_dir):
        ...
        if self.do_bg and self.obj_id == 0:
            ...
            sampled_ray_dirs[:,:,1]  = sampled_ray_dirs[:,:,1] * -1 
            sampled_ray_dirs[:,:,2]  = sampled_ray_dirs[:,:,2] * -1 
        else:
