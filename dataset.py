...
class Replica(Dataset):
    def __init__(self, cfg):
        ...
        self.Twc = np.loadtxt(traj_file, delimiter=" ").reshape([-1, 4, 4])#The camera poses are stored as 4x4 transformation matrix in homogenous-coordinate:
        self.depth_transform = transforms.Compose( #串联多个图片变换的操作。
            [image_transforms.DepthScale(cfg.depth_scale),#*(1/scale)
             image_transforms.DepthFilter(cfg.max_depth)])#屏蔽大于max_depth的值
    ...
    def __getitem__(self, idx):
        ...
        if self.imap_mode:
            ...
            inst_list = []
            batch_masks = []
            for inst_id in np.unique(inst):#去重
                inst_mask = inst == inst_id#indicates the pixels that belong to the current instance.
                ...
                if sem_cls in self.background_cls_list: #ignore background
                    continue
